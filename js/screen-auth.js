...
function ProfileSelect(p){
  ...
  function del(id,e){
    ...
    try{["progress","history","paused"].forEach(function(k){localStorage.removeItem(getKey(id,k));});}catch(e){}
  }
    ...
    profiles.length>0&&React.createElement("div",{style:{marginBottom:"16px"}},
      profiles.map(function(pr){
        ...
        return React.createElement("button",{key:pr.id,onClick:function(){pick(pr);},style:cs({display:"flex",alignItems:"center",gap:"14px",width:"100%",marginBottom:"10px",cursor:"pointer",textAlign:"left",border:"1px solid "+BORDER,padding:"14px 16px"})},
          ...
          React.createElement("button",{onClick:function(e){del(pr.id,e);},style:{background:"none",border:"none",color:MUTED,fontSize:"16px",cursor:"pointer",padding:"4px 8px",flexShrink:0}},"🗑"),
...
function ProfileCreate(p){
  ...
  function create(){
    ...
    if(prs.some(function(pr){return pr.name.toLowerCase()===n.toLowerCase();})){setErr("That name is already taken.");return;}
    var pr={id:uid(),name:n,avatar:avatar,createdAt:new Date().toISOString()};
    ...
    React.createElement("div",{style:cs({marginBottom:"14px"})},
      ...
      name.trim()&&React.createElement("div",{style:{display:"flex",alignItems:"center",gap:"12px",background:"rgba(255,209,102,.07)",borderRadius:"12px",padding:"12px",marginBottom:"14px"}},
        ...
        React.createElement("div",null,
          ...
          React.createElement("div",{style:{color:MUTED,fontSize:"11px"}},"Ready to start!")
        )
      ),
      React.createElement("button",{onClick:create,style:bs("linear-gradient(135deg,"+GOLD+","+ORANGE+")",{width:"100%",color:BG,fontSize:"15px",padding:"13px"})},"Create Profile & Start")
    )
  );
}
